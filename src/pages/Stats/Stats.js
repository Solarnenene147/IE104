...
const imageContext = require.context('../../images/partner', false, /\.(png|jpe?g|svg)$/);
...
function Stats() {
  ...
  const calculateRaisedAmount = (orgName) => {
    const relevantCharities = data.charities.filter(charity => charity.organization === orgName);
  ...
  const orgsToDisplay = sortedOrgs.slice((currentOrgPage - 1) * ITEMS_PER_PAGE, currentOrgPage * ITEMS_PER_PAGE);
  const usersToDisplay = sortedUsers.slice((currentUserPage - 1) * ITEMS_PER_PAGE, currentUserPage * ITEMS_PER_PAGE);
          ...
          {orgsToDisplay.map((item, index) => {
            ...
            return (
              <div key={item.organization} className='org_entry'>
                ...
                {index === 0 && currentOrgPage === 1 && <img src={'https://img2.annthegran.com/printart/xlarge/maya_kreations/pgmks1168.webp'} alt="Crown" className="crown_icon_org" />}
                <img src={org.image} alt={`${item.organization} logo`} className="img_org_ranking" />
                </div>
                <div className='org_info'>
                  <p>Tên tổ chức: <span>{item.organization}</span></p>
                  <p>Tổng số tiền đã kêu gọi: {totalRaisedAmount}đ</p>
                  <p>Số dự án đã hoàn thành: {item.completedProjects}</p>
                  <p>Số dự án đang trong tiến hành: {item.inProgressProjects}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className='entry'>
          {usersToDisplay.map((user, index) => (
            <div key={user.name} className="user_entry">
              <div className='avatar_wrapper'>
                {index === 0 && currentUserPage === 1 && <img src={'https://img2.annthegran.com/printart/xlarge/maya_kreations/pgmks1168.webp'} alt="Crown" className="crown_icon" />}
                <img src={user.avatar} alt={`${user.name}'s avatar`} className="img_user_ranking" />
              </div>
